...
<body class="app">   	
  ...
  <circle cx="3.5" cy="10.5" r=".5"/>
	  ...
	  <path fill-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"/>
						...
						<li class="nav-item">
							<a class="nav-link" href="{{route('dashboard', ['view' => 'etablissements'])}}">
...
@endif
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalAddRemboursementLabel">Enregistrer une Cotisation</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header"  class="forced-colors:bg-green-500">
                <h5 class="modal-title" id="modalEtablissementLabel">Enregistrer une Cotisation</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" ></button>
            </div>
            <div class="modal-body">
...
function remplirModaleEdit(user) {
    // 1. On définit l'URL dynamiquement (ex: /users/5)
...
// Configuration commune pour minimiser les graphes
const commonOptions = {
    maintainAspectRatio: false,
    cutout: '75%', // Look moderne (anneau fin)
    plugins: {
        legend: {
            position: 'bottom',
...
new Chart(document.getElementById('statutCotisationChart'), {
    type: 'doughnut',
    data: {
...
new Chart(document.getElementById('statutRemboursementChart'), {
    type: 'doughnut',
    data: {
        labels: Object.keys(dataRemb),
        datasets: [{
            data: Object.values(dataRemb),
            backgroundColor: ['#0d6efd', '#f59e0b', '#dc3545'], // Bleu, Orange, Rouge
            borderWidth: 2
        }]
    },
    options: commonOptions
...
<script>
    ...
    responsive: true,
    plugins: {
        legend: { display: false } // On cache la légende pour plus de clarté
    },
    scales: {
        x: { grid: { display: false } }, // Supprime la grille verticale
        y: { beginAtZero: true, grid: { borderDash: [5, 5] } } // Grille pointillée horizontale
    }
};

// Exemple pour les Cotisations (Bar Chart)
new Chart(document.getElementById('cotisationChart'), {
    type: 'bar',
    data: {
        labels: Object.keys(cotisations),
        datasets: [{
            label: 'Montant ($)',
            data: Object.values(cotisations),
            backgroundColor: '#3b82f6', // Bleu Tailwind
            borderRadius: 8, // Barres arrondies comme sur tes images
            barThickness: 20
        }]
    },
    options: commonOptions
...
// Exemple pour les Remboursements (Line Chart avec dégradé)
const ctxRemb = document.getElementById('remboursementChart').getContext('2d');
const gradient = ctxRemb.createLinearGradient(0, 0, 0, 400);
...
new Chart(ctxRemb, {
    type: 'line',
    data: {
        labels: Object.keys(remboursements),
        datasets: [{
            data: Object.values(remboursements),
            borderColor: '#3b82f6',
            fill: true,
            backgroundColor: gradient, // Effet d'aire sous la courbe
...
// Configuration des courbes (Line Chart)
const lineOptions = {
    maintainAspectRatio: false,
    ...
    elements: {
        line: { tension: 0.4 } // Rend la courbe lisse (smooth)
    }
};

// Graphique Courbe Cotisations
new Chart(document.getElementById('cotisationsMensuellesChart'), {
    type: 'line',
    data: {
        labels: labelsMois,
        datasets: [{
...
// Graphique Courbe Remboursements
new Chart(document.getElementById('remboursementsMensuelsChart'), {
    type: 'line',
    data: {
        labels: labelsMois,
        datasets: [{
